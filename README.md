The original Forum software most other message boards are based on!

YaBB (Yet Another Bulletin Board) first came on the scene on June 16, 2001, when teenager Zef Hemel and a team of collaborators released YaBB 1.0  

It was notable as one of the very first open-source internet forum packages written in Perl.

and now 25 years later it's still here and better than ever.

Currently over 50,000 web communities using YaBB worldwide, and over 1 Million registered users througout these forums!

Download from Trunk, chown the yabb folder to apache:yourusername for EL or www-data:yourusername for Debian then run Setup.pl

For help go to https://yabbforumsoftware.com/cgi-bin/yabb2/YaBB.pl or https://myforum.nz

To contribute to developement or help in any way use the contacts on the sites above.

