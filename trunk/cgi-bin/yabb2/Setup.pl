...
$setupplver = 'YaBB 2.6.0 $Revision: 1503 $';
$yymycharset  = 'UTF-8';
...
EOF
    ...
    if ( !-e "$varsdir/news.txt" ) {
        ...
        print {NEWSFILE} "We've upgraded to YaBB 2.6.0!\n"
          or croak 'cannot print news.txt';
        print {NEWSFILE}
...
~;
    ...
    $yyim    = 'You are running YaBB 2.6.0 Setup.';
    $yytitle = 'YaBB 2.6.0 Setup';
    SetupTemplate();
    return;
}
...
<div class="bordercolor borderbox">
    ...
    <table class="border-space pad-cell">
        <tr>
            <td class="windowbg">
                Here you can set some of the default settings for your new YaBB 2.6.0 forum.<br />
...
'We are currently upgrading our forum again. Please check back shortly!';
        ...
        $max_log_days_old       = 90;
        $fadertime              = 1000;
        $defaultusertxt         = 'I Love YaBB 2.6.0!';
        $timeout                = 5;
        $HotTopic               = 10;
        $VeryHotTopic           = 25;
        $barmaxdepend           = 0;
...
<div class="boardcontainer">
    ...
    if ($set_created) {
        $yymain .= qq~Successfully checked Settings.pm!
            ...
            <td class="windowbg center"><img src="$imagesdir/check.png" alt="" /></td>
            <td class="windowbg2">
      Click on 'Continue' and go to your <i>Admin Center - Forum Settings</i> to set the options for your YaBB 2.6.0 forum.<br />Or to convert a 1x or 2x Forum to 2.6.0
            </td>
        </tr>~;
    }
...
~;
      $yyim    = "YaBB 2.6.0 Setup Error.";
      $yytitle = "YaBB 2.6.0 Setup Error.";
...
sub FoundSetupLock {
    ...
    if ( -e "$vardir/Converter.lock" ) {
        $conv = q{};
        $conv2 =
qq~The 1x to 2.6.0 Converter has already been run.<br />To run the Converter again, remove the file "$vardir/Converter.lock," then re-visit this page.~;
...
<!--                  <input type="hidden" name="formsession" value="$formsession" />-->
    ...
    $yyim    = 'YaBB 2.6.0 Setup has already been run.';
    $yytitle = 'YaBB 2.6.0 Setup';
    template();
    return;
}
