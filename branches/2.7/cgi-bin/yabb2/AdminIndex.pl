...
what_language();        # Figure out which language file we should be using!
get_micon();
...
sub yymaina {
    ...
    if ( $maintenance && $action && $action eq 'login2' ) {
    ...
    if ( !$iamadmin && !$iamgmod ) {
        if ($maintenance) { require Sources::LogInOut; in_maintenance(); }
...
sub parsenavarray {
    ...
    foreach my $element (@x) {
        chomp $element;
        my ( $action_to_take, $vistext, $whatitdoes, $isheader ) =
          split /[|]/xsm, $element;
        my $currentclass = q{};
        if ( $action_area && $action_area eq $action_to_take ) {
        ...
        if ( $iamgmod && !$gmod_access{$action_to_take} ) {
            next;
        }
...
sub admimgloc {
    my ($img) = @_;
    my $thisimgloc = qq~img src="$imagesdir/$img"~;
...
sub admimgloc2 {
    my ($img) = @_;
    my $thisimgloc = qq~$imagesdir/$img~;
    if ( !-e "$htmldir/Templates/Forum/$useimages/$img" ) {
...
sub admintemplate {
    my $adminstyle =
qq~<link rel="stylesheet" href="$yyhtml_root/Templates/Admin/default.css" type="text/css" />~;

    my $adminimages = qq~$yyhtml_root/Templates/Admin/default~;
    require "$templatesdir/admin/AdminCentre.template";
    require Variables::Gmodset;
...
### END BOARDMOD ANCHOR ###
    ...
    my $boardlink = qq~$boardurl/$yyexec.$yyext~;

    if ($accept_permafull) {
        $boardlink = qq~$perm_domain/$symlink/~;
    }
    my $topmenu_one = qq~<a href="$boardlink">$admintxt{'15'} $mbname</a>~;
    my $topmenu_two = qq~<a href="$adminurl">$admintxt{'33'}</a>~;
    my $topmenu_tree =
