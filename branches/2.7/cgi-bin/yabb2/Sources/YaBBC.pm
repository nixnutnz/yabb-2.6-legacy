...
sub MakeSmileys {
    ...
    (my $smname = $message) =~ s~\[smil(ie|ey)=(\S+?\.(gif|jpg|png|bmp))\]~$2~gism;
    my ( $smileydesc, undef ) = split /\./xsm, $smname;
    $message =~ s~(\W|^)\[smil(ie|ey)=(\S+?\.(gif|jpg|png|bmp))\]~$1<img class="smil" data-rel="\[smil$2=$3\]" src="$yyhtml_root/Smilies/$3" alt="$post_txt{'287'}" title="$smileydesc" />~gism;
