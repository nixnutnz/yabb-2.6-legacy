...
## system ##
our (
    $allow_gmod_profile, $codecount,            $css,
    $date,               $g_count,              $g_newmessage,
    $iamadmin,           $iamfmod,              $iamgmod,
    $iamguest,           $imsend,               $language,
    $mbccusers,          $mccusers,             $menusep,
    $message_flags,      $messageid,            $mstatus,
    $mtousers,           $musername,            $newload,
    $page,               $pageindex1,           $pageindexjs,
    $enable_guest_alert, $quotecount,           $quotemsg,
    ...
    @messim,             $view,                 $destination,

);

## CP/Post section ##
our ( $replyguest, $mc_globalformstart, $mctitle, $bc_count, $bc_newmessage,
    $post, );
## template ##
our (
...
## local - more than 1 sub ##
my (
    $acount,      $callerid,    $mc_content, $mc_pmmenu,    $mc_postsmenu,
    $mc_profmenu, $mc_viewmenu, $movebutton, $norm_dateset, $senderinfo,
);

my $pm_lev = pm_lev();
my ( $show_pm, $im_box, $pmfile_toopen, $is_bm_mess ) = ( q{}, q{}, q{}, q{} );
our ( $send_bm_mess, $show_profile, $show_favorites, $show_notifications ) =
  ( q{}, q{}, q{}, q{} );
our @dimmessages;
...
sub mycenter {
    ...
    my %get_view = (
        'deletemultimessages' => [ 'pm',          q{} ],
        'pmsearch'            => [ 'pm',          q{} ],
        'mycenter'            => [ 'mycenter',    q{} ],
        'shownotify'          => [ 'notify',      "$img_txt{'418'}" ],
        'boardnotify2'        => [ 'notify',      "$img_txt{'418'}" ],
        'myusersrecentposts'  => [ 'recentposts', q{} ],
        'favorites'           => [ 'favorites',   "$img_txt{'70'}" ],
    );

## mycenter view mod hook ##

    if ( exists $get_view{$action} ) {
        my ( $getview, $mctitl ) = @{ $get_view{$action} };
        $view = $getview;
        if ($mctitl) { $mctitle = $mctitl; }
    }
    elsif ( $action =~ /^im/xsm ) { $view = 'pm'; }
    elsif ( $action =~ /^notify/xsm ) {
        $view    = 'notify';
        $mctitle = $img_txt{'418'};
    }
...
sub check_messageflag {
    ...
    if ( %{ 'MF' . $user . $pm_file } ) {
        ...
        foreach (@usermessages) {
            ...
            if (   $umessage_id
                && $umessage_id == $id
                && $umessage_flags =~ /$message_flag/ixsm )
            {
                $message_foundflag = 1;
            }
        }
...
sub update_pms {
    ...
    my %im_count = (
        'messagein'  => \&im_count_messagein,
        'messageout' => \&im_count_messageout,
        'inread'     => \&im_count_inread,
        'callback'   => \&im_count_callback,
        'draftadd'   => \&im_count_draftadd,
        'draftsend'  => \&im_count_send,
    );
    if ( $target && $im_count{$target} ) {
        $im_count{$target}( $user, $id );
    }
...
sub del_some_im {
    ...
    if (   $FORM{'imaction'} eq $inmes_txt{'remove'}
        ...
        || $INFO{'deleteid'} )
    {
        del_im_1( $username, \@messages, $file_toopen );
    }
    elsif ($FORM{'imaction'} eq $inmes_imtxt{'store'}
        || $INFO{'imaction'} eq $inmes_imtxt{'store'} )
    {
        del_im_2( $username, \@messages, $file_toopen );
    }

    my $redirect = 'im';
    my %getre    = (
        '2' => 'imoutbox',
        '3' => "imstorage;viewfolder=$INFO{'viewfolder'}",
        '4' => 'imdraft',
        '5' => 'im;focus=bmess',
        '6' => 'im;focus=gmess',
    );
    if ( $getre{ $INFO{'caller'} } ) {
        $redirect = $getre{ $INFO{'caller'} };
    }
...
sub create_userdisplay_line {
    ...
    my $usernamelink = qq~<b>$usrname</b>~;
    if ( $yy_udloaded{$usrname} ) {
        ...
        {
            our $signature = ${ $uid . $usrname }{'signature'};
...
qq~$menusep<a href="mailto:${$uid.$usrname}{'email'}">$img{'email_sm'}</a>~;
            ...
            my @conlist =
              qw( weburl gtalk skype myspace facebook twitter youtube icq yim aim );
            our $memb_adinfo = q{};
            foreach my $i (@conlist) {
                if ( ${ $uid . $usrname }{$i} ) {
                    $memb_adinfo .= $menusep . ${ $uid . $usrname }{$i};
                }
            }
        }
...
sub draw_pmbox {
    ...
    {
        @dimmessages =
          get_mycenter_mess( \@dimmessages, $pmfile_toopen, $username );
    }
...
~;
    ...
    my %unlink = (
        'im'        => "$memberdir/$username.msg",
        'imoutbox'  => "$memberdir/$username.outbox",
        'imstorage' => "$memberdir/$username.imstore",
        'imdraft'   => "$memberdir/$username.imdraft",
    );
    if (   $action =~ /^im/xsm
    ...
    my $display_prof         = 'none';
    my $tab_prof_highlighted = 'windowbg';
    if (
        $view eq 'profile'
        || (
            $view eq 'mycenter'
    ...
    my $display_posts          = 'none';
    my $tab_notify_highlighted = 'windowbg';
    if (   $view eq 'notify'
        || $view eq 'favorites'
        || $view eq 'recentposts' )
    {
    ...
    my $display_pm         = 'none';
    my $tab_pm_highlighted = 'windowbg';
    if (
        $view eq 'pm'
        || (   $view eq 'mycenter'
            && $pm_lev == 1 )
...
## start Profile div
    ...
    if ( !$sid ) {
        $sid          = $FORM{'sid'};
        $profile_link = 'action=profileCheck;page=';
    }
    if ($sid) { $sid_link = ";sid=$sid"; $profile_link = 'action='; }

    my %thislink = (
        'a' => "action=myviewprofile;username=$useraccount{$username}",
        'b' => "$profile_link"
          . 'myprofile;username='
          . "$useraccount{$username}$sid_link",
        'c' => "$profile_link"
    ...
    if ($enable_buddylist) {
        ...
        $my_buddylink =~ s/\Q{yabb thisLink_e}/$thislink{'e'}/xsm;
    }

    my $my_impref = q{};
    if ( $pm_lev == 1 ) {
        $my_impref = $my_thislink_impref;
        $my_impref =~ s/\Q{yabb thisLink_f}/$thislink{'f'}/xsm;
    }
      ...
      )
    {
        ...
        $my_adminlink =~ s/\Q{yabb thisLink_g}\E/$thislink{'g'}/xsm;
    }

    $mc_profmenu = $my_mc_profmenu;
    $mc_profmenu =~ s/\Q{yabb display_prof}\E/$display_prof/xsm;
    $mc_profmenu =~ s/\Q{yabb thisLink_a}\E/$thislink{'a'}/xsm;
    $mc_profmenu =~ s/\Q{yabb thisLink_b}\E/$thislink{'b'}/xsm;
    $mc_profmenu =~ s/\Q{yabb thisLink_c}\E/$thislink{'c'}/xsm;
    $mc_profmenu =~ s/\Q{yabb thisLink_d}\E/$thislink{'d'}/xsm;
...
qq~<script src="$yyhtml_root/ubbc.js" type="text/javascript"></script><form action="$scripturl?action=$destination" method="post" name="postmodify" id="postmodify" enctype="application/x-www-form-urlencoded">~;
    ...
    ###################################################
    my %doprofile = (
        'myprofileIM'        => \&modify_profile_pm,
        'myprofileIM2'       => \&modify_profile_pm2,
        'myprofile'          => \&modify_profile,
        'myprofile2'         => \&modify_profile2,
        'myprofileContacts'  => \&modify_profile_contacts,
        'myprofileContacts2' => \&modify_profile_contacts2,
        'myprofileOptions'   => \&modify_profile_options,
        'myprofileOptions2'  => \&modify_profile_options2,
        'myprofileBuddy'     => \&modify_profile_buddy,
        'myprofileBuddy2'    => \&modify_profile_buddy2,
        'myviewprofile'      => \&view_profile,
        'myprofileAdmin'     => \&modify_profile_admin,
        'myprofileAdmin2'    => \&modify_profile_admin2,
    );
    if ( $view eq 'mycenter' ) {
        $mc_content .= view_mycenter($username);
    }
    elsif ( $view eq 'pm' ) {
        if ( $action eq 'imsend' || $action eq 'imsend2' ) {
            ...
            $mc_content =~
              s/\Q{yabb MCGlobalFormStart}\E/$mc_globalformstart/xsm;
        ...
        elsif ( $action eq 'imshow' ) { draw_pmview2(); }
        elsif ( $action eq 'pmsearch' ) {
            spam_protection();
...
##Profile Mod Hook ##
    ...
    ## start PM div
    if ( $pm_lev == 1 ) {
        $mc_pmmenu = get_mc_pmmenu($display_pm);
    }
    ## end PM div
    return;
}
...
sub draw_pmview {
        ...
        || ( $storecontentfound == 0 && $INFO{'viewfolder'} ) )
    {
        ...
        # work out the newest pm date so as to put the right first block in
        my $uselegend   = q{};
        my $gt_monthset = 0;
        ...
        foreach my $counter ( $start .. ( $maxcounter - 1 ) ) {
            ...
            my $usernameto      = q{};
            my $usertomess_read = q{};
            if (   $action eq 'imoutbox'
                || $action eq 'imstorage'
                || $action eq 'imdraft' )
            {
                if ($has_multirecs) {
                    ( $usernameto, $usertomess_read ) = multi_1( \%messlst );
                }
                else {
                    ( $usernameto, $usertomess_read ) = multi_2( \%messlst );
                }
            }
                ...
                && !$view_gmess )
            {
                ...
                if ( $action eq 'im' ) {
                    ## not opened
                    $message_icon =
                      get_im_icon( $has_multirecs, $im_opened, $im_repliedto );
                }

                ##  outbox
                elsif ( $action eq 'imoutbox' ) {
                    ( $message_icon, $call_back ) = get_out_icon(
                        $has_multirecs, $im_opened,
                        $im_repliedto,  \%messlst
                    );
                    ## not opened
                }
            }
            ...
            if ( $sort_by eq 'gpdate' ) {
                my $gpdate = ( $date - $messlst{'mdate'} ) / 86400;
                $uselegend = get_legend(
                    $gpdate,        $latest_dateset, $counter,
                    $counter_check, $two_weeksset,   $three_weeksset,
                    $monthset,      $last_weekset,   $gt_monthset
                );
                $mc_content_lgnd = q{};
                if ($uselegend) {
...
qq~$guest_name<br />(<a href="mailto:$guest_email">$guest_email</a>)~;
                ...
                my @usernameto = ();
                if ( $messlst{'mstatus'} =~ /g/xsm ) {
                    ( @usernameto, $usernamefrom ) = get_guest_to( \%messlst );
...
qq~$guest_name<br />(<a href="mailto:$guest_email">$guest_email</a>)~;
                    ...
                    (@usernameto) = get_more_to( \%messlst );
                    $usernameto = join q{, }, @usernameto;
                    load_user( $messlst{'musername'} );    # is from user
                    $usernamefrom =
                      ${ $uid . $messlst{'musername'} }{'realname'}
...
qq~<a href="$scripturl?action=deletemultimessages;caller=$callerid;deleteid=$messlst{'messageid'}$storefolder_view" onclick="return confirm('$inmes_txt{'770'}')">$inmes_txt{'remove'}</a>~;
                ...
                elsif ($messlst{'mstatus'} =~ /g/xsm
                    && $messlst{'mstatus'} !~ /gr/xsm )
                {
                    $actions_menu =
qq~<a href="$scripturl?action=imsend;caller=$callerid;quote=$messlst{'mreplyno'};replyguest=1;id=$messlst{'messageid'}">$inmes_txt{'146'}</a>~;
                }
...
qq~<img src="$imagesdir/$im_code1" alt="$inmes_imtxt{'84'}" title="$inmes_imtxt{'84'}" />&nbsp;~;
                ...
                my $cliped     = 0;
                ( $immessage, $cliped ) =
                  count_chars( $immessage, $convertcut );
                $immessage = to_chars($immessage);
                ...
                if ( $immessage !~ /\x23nosmileys/ixsm ) {
                    enable_yabbc();
                    $immessage = make_smileys($immessage);
                }
...
qq~<input type="submit" name="imaction" value="$inmes_txt{'remove'}" class="button" onclick="return confirm('$inmes_txt{'delmultipms'}$mattach_deletewarn');" />~;
        ...
        <span class="small"><b>$imbargfx&nbsp;$intext</b><br /><br /></span>~;
            ...
            if (
                ...
                && !(
                    $action eq 'imstorage'
                    && ( !$INFO{'viewfolder'} || $INFO{'viewfolder'} eq q{} )
                )
              )
            {
...
sub mc_menu {
    ...
    my %class = (
        ...
        'imsend2'            => 'pmclass',
        'imshow'             => 'pmclass',
        'profileCheck'       => 'profclass',
        'myviewprofile'      => 'profclass',
        'myprofile'          => 'profclass',
        'myprofileContacts'  => 'profclass',
        'myprofileOptions'   => 'profclass',
        'myprofileBuddy'     => 'profclass',
        'myprofileIM'        => 'profclass',
        'myprofileAdmin'     => 'profclass',
        'favorites'          => 'postclass',
        'shownotify'         => 'postclass',
        'myusersrecentposts' => 'postclass',
    );
    if ( $class{$action} eq 'pmclass' ) {
    ...
    elsif ( $class{$action} eq 'profclass' ) {
        $profclass = q~ class="selected"~;
    }
    elsif ( $class{$action} eq 'postclass' ) {
...
<li><span onclick="changeToTab('posts'); return false;"$postclass  id="menu_posts"><a href="$scripturl?action=favorites" onclick="changeToTab('posts'); return false;">$mc_menus{'posts'}</a></span></li>
    ~;
    return;
}
...
sub im_count_inread {
    my ( $usr, $id ) = @_;
    if ( update_messageflag( $usr, $id, 'msg', 'u', q{} ) ) {
        ${$usr}{'PMimnewcount'}--;
        return;
    }
    else { return; }
...
sub im_count_draftadd {
    my ( $usr, undef ) = @_;
    ${$usr}{'PMdraftnum'}++;
    return;
}

sub im_count_draftsend {
    my ( $usr, undef ) = @_;
    ${$usr}{'PMdraftnum'}--;
    return;
}
...
sub get_im {
    my ($usr) = @_;
    if ( $INFO{'caller'} < 5 ) {
        update_pms( $usr, $INFO{'id'}, 'inread' );
    }
    my $msubj  = q{};
    my $messge = q{};
    our ( $qmessageid, $mfrom, $mto, $mreplyno, $mdate, $mip, );
    my $pm_filetype = "$usr.msg";
    if ( $INFO{'caller'} && exists $filetoopen{ $INFO{'caller'} } ) {
    ...
    foreach my $checkthemessage (@messages) {
        (
            $qmessageid, $mfrom,   $mto,    undef, undef,
            $msubj,      $mdate,   $messge, undef, $mreplyno,
            $mip,        $mstatus, undef,   undef, undef
...
s/\n{0,1}\[quote([^\]]*)\](.*?)\[\/quote([^\]]*)\]\n{0,1}/\n/igxsm;
            ...
            if ( $INFO{'forward'} || $INFO{'quote'} ) {
                $messge =
qq~[quote author=$cloaked_author link=impost date=$mdate\]$messge\[/quote\]\n~;
            }
            our $nscheck = q{};
            if ( $messge =~ /\x23nosmileys/ixsm ) {
                $messge =~ s/\x23nosmileys//igxsm;
...
sub getfolder {
    my ($usr) = @_;
    my $folder = q{};
    if ( -e "$memberdir/$usr.imstore" ) {
        our ($THREADS);
        fopen( 'THREADS', '<', "$memberdir/$usr.imstore" )
          or croak "$croak{'open'} $usr.imstore";
        my @threads = <$THREADS>;
        fclose('THREADS') or croak "$croak{'close'} $usr.imstore";
        my $threadid = $INFO{'id'};
...
sub getimbox {
    ...
    if (   $action eq 'im'
        ...
        || $action eq 'imstorage' )
    {
        $other_storeselect .= getimbox_a($username);
    }
    ...
    my %get_ims = (
        'imdraft' => [
            q{},                    $inmes_txt{'draft'},
            $inmes_imtxt{'status'}, $inmes_txt{'324'},
            4,                      $inmes_txt{'draft'},
            q{},                    $inmes_txt{'draft'},
            'imdraft'
        ],
...
qq~<input type="submit" name="imaction" value="$inmes_imtxt{'store'}" class="button" />$other_storeselect $inmes_txt{'storeor'}~,
    ...
    ( $status, $boxtxt ) = getimbox_b( $action, \%get_ims, $other_storeselect );
    return;
}

sub del_im_1 {
    my ( $usr, $messages, $file_toopen ) = @_;
    ...
    foreach (@messages) {
        ...
        if ( !exists $FORM{ 'message' . $m[0] } ) {
            push @delpost, $_;
            %countstore = del_im_a( $username, \@m, \%countstore );
        }
        else {
            del_im_b( $username, \@m );
        }
    ...
    if ( $INFO{'caller'} == 3 ) {
        ${$usr}{'PMfoldersCount'} = q{};
        ${$usr}{'PMstorenum'}     = 0;
        foreach ( split /[|]/xsm, ${$usr}{'PMfolders'} ) {
            $countstore{$_} ||= 0;
            ${$usr}{'PMfoldersCount'} .=
              ${$usr}{'PMfoldersCount'} eq q{}
              ? $countstore{$_}
              : "|$countstore{$_}";
            ${$usr}{'PMstorenum'} += $countstore{$_};
        }
    }
    build_ims( $usr, 'update' );
    return;
}
...
sub del_im_2 {
    ...
    if (@newmessages) {
        if ( $INFO{'caller'} != 3 ) {
            if ( -e "$memberdir/$usr.imstore" ) {
                our ($IUSRFILE);
                fopen( 'IUSRFILE', '<', "$memberdir/$usr.imstore" )
        ...
        our ($TRANSFER);
        fopen( 'TRANSFER', '>', "$memberdir/$usr.imstore" )
...
sub getdrawpmbox_1 {
    my ( $usr, $toopen ) = @_;
    my ( @dmmessages, @gmessages, @bmessages );
    if ( $callerid < 5 ) {
        if ( -e "$memberdir/$usr.$toopen" ) {
            our ($NFILE);
            fopen( 'NFILE', '<', "$memberdir/$usr.$toopen" )
              or croak "$croak{'open'} NFILE";
            @dmmessages = <$NFILE>;
            fclose('NFILE') or croak "$croak{'close'} NFILE";
            foreach ( reverse sort @dmmessages ) {
                my (
                    $m_id, undef, undef, undef, undef, undef,   undef,
                    undef, undef, undef, undef, undef, $m_flag, undef
                ) = split /[|]/xsm, $_, 14;
                ${ $usr . $toopen }{$m_id} = $m_flag;
    ...
    return ( @dmmessages, @gmessages, @bmessages );
}
...
sub view_mycenter {
    my ($usr) = @_;
    load_user_display($usr);

    my $onoffstatus =
      ${ $uid . $usr }{'offlinestatus'} eq 'away'
    ...
    my $memberinfo  = "$memberinfo{$usr}$addmembergroup{$usr}";
    my $user_online = user_onlinestatus($usr) . q~<br />~;
    my $template_postinfo =
qq~$mycenter_txt{'posts'}: <a href="$scripturl?action=myusersrecentposts;username=$useraccount{$usr}" title="$mycenter_txt{'mylastposts'}">~
      . number_format( ${ $uid . $usr }{'postcount'} )
      . q~</a><br />~;
    my $template_age;
    if (   ${ $uid . $usr }{'bday'}
        && $showuserage
    ...
    $myprofileblock =~ s/\Q{yabb userlink}\E/$link{$usr}/gxsm;
    $myprofileblock =~ s/\Q{yabb memberinfo}\E/$memberinfo/gxsm;
    $myprofileblock =~ s/\Q{yabb stars}\E/$memberstar{$usr}/gxsm;
    $myprofileblock =~ s/\Q{yabb useronline}\E/$user_online/gxsm;
    $myprofileblock =~ s/\Q{yabb userpic}\E/${ $uid . $usr }{'userpic'}/gxsm;
    $myprofileblock =~ s/\Q{yabb usertext}\E/${ $uid . $usr }{'usertext'}/gxsm;
    $myprofileblock =~ s/\Q{yabb postinfo}\E/$template_postinfo/gxsm;
    $myprofileblock =~ s/\Q{yabb location}\E/$userlocation/gxsm;
    $myprofileblock =~ s/\Q{yabb gender}\E/${ $uid . $usr }{'gender'}/gxsm;
    $myprofileblock =~ s/\Q{yabb zodiac}\E/${ $uid . $usr }{'zodiac'}/gxsm;
...
## Mod Hook myprofileblock ##
    ...
    if ($enable_buddylist) {
        if (   ${ $uid . $usr }{'buddylist'}
            && ${ $uid . $usr }{'buddylist'} ne q{} )
        {
    ...
    my $mccontent = $my_mc_content;
    $mccontent =~ s/\Q{yabb myprofileblock}\E/$myprofileblock/xsm;
    $mccontent =~ s/\Q{yabb buddiesCurrentStatus}\E/$buddies_currentstatus/xsm;
    $mccontent =~ s/\Q{yabb onOffStatus}\E/$onoffstatus/xsm;
    $mccontent =~ s/\Q{yabb stealthstatus}\E/$stealthstatus/xsm;
...
sub get_mc_pmmenu {
    ...
    my ( $mc_pmmenu_bmbox, $mc_pmmenu_gmbox ) = view_bgmess();
    ...
    my $mc_pmmenu_newfolder = q{};
    my $mc_pmmenu_strtot    = q{};
    if ( $enable_storefolders > 0 ) {
        ( $mc_pmmenu_strtot, $mc_pmmenu_newfolder ) =
          view_folders(@folder_count);
    }
    my $mc_pmmenu_markall = $my_markall;
    $mc_pmmenu_markall =~ s/\Q{yabb MCPmMenu_strtot}\E/$mc_pmmenu_strtot/xsm;
...
qq~\nvar markallreadlang = '$inmes_txt{'500'}';\nvar markfinishedlang = '$inmes_txt{'500a'}';~;
    ...
    $enable_pm_search ||= 0;
    my $mc_pmmenu_pmsearch = q{};
    if ( $enable_pm_search > 0 ) {
        my $mc_pmmenu_pmsearch_b = q{};
    ...
    my $mc_pmmen = $my_mc_pmmenu;
    $mc_pmmen =~ s/\Q{yabb display_pm}\E/$display_pm/xsm;
    $mc_pmmen =~ s/\Q{yabb mypmmmenu}\E/$mypmmmenu_imsend/xsm;
    $mc_pmmen =~ s/\Q{yabb MCPmMenu_bm}\E/$mc_pmmenu_bm/xsm;
    $mc_pmmen =~ s/\Q{yabb mypmmenu_inbox}\E/$mypmmenu_inbox/xsm;
    $mc_pmmen =~ s/\Q{yabb username_PMmnum}\E/${$username}{'PMmnum'}/xsm;
    $mc_pmmen =~ s/\Q{yabb inboxNewCount}\E/$inbox_newcount/xsm;
    $mc_pmmen =~ s/\Q{yabb MCPmMenu_bmbox}\E/$mc_pmmenu_bmbox/xsm;
    $mc_pmmen =~ s/\Q{yabb MCPmMenu_gmbox}\E/$mc_pmmenu_gmbox/xsm;
    $mc_pmmen =~ s/\Q{yabb foldercount0}\E/$foldercount0/xsm;
    $mc_pmmen =~ s/\Q{yabb foldercount1}\E/$foldercount1/xsm;
    $mc_pmmen =~ s/\Q{yabb PMdraftnum}\E/${$username}{'PMdraftnum'}/xsm;
    $mc_pmmen =~ s/\Q{yabb PMmoutnum}\E/${$username}{'PMmoutnum'}/xsm;
    $mc_pmmen =~ s/\Q{yabb PMstorenum}\E/${$username}{'PMstorenum'}/xsm;
    $mc_pmmen =~ s/\Q{yabb MCPmMenu_markall}\E/$mc_pmmenu_markall/xsm;
    $mc_pmmen =~ s/\Q{yabb MCPmMenu_newfolder}\E/$mc_pmmenu_newfolder/xsm;
    $mc_pmmen =~ s/\Q{yabb MCPmMenu_pmsearch}\E/$mc_pmmenu_pmsearch/xsm;

    return $mc_pmmen;
}

sub del_im_a {
    my ( $usr, $m, $countstore ) = @_;
    my @m          = @{$m};
    my %countstore = %{$countstore};
    if    ( $INFO{'caller'} == 2 ) { ${$usr}{'PMmoutnum'}++; }
...
sub del_im_b {
    ...
    if ( $INFO{'caller'} == 3 ) {
        ...
        if ( ${$usr}{'PMbcRead'} !~ s/\b$m[0]$//gxsm ) {
            ${$usr}{'PMbcRead'} =~ s/$m[0]\b//gxsm;
        }
    }
    elsif ( $INFO{'caller'} == 6 ) {
        if ( ${$usr}{'PMgRead'} !~ s/\b$m[0]$//gxsm ) {
            ${$usr}{'PMgRead'} =~ s/$m[0]\b//gxsm;
        }
    }
    return;
}
...
sub getimbox_a {
    ...
    my $foundextra = 0;
    foreach my $storefolder ( split /[|]/xsm, ${$usr}{'PMfolders'} ) {
        if (  !$INFO{'viewfolder'}
...
qq~ $inmes_txt{'storein'} <select name="tostorefolder" id="tostorefolder">~;
        foreach my $other_folder (@other_storefolders) {
            ...
            if ( $other_folder eq 'in' || $other_folder eq 'out' ) {
                $other_foldername = $im_folders_txt{$other_folder};
            }
            $other_storeselect .=
    ...
    return $other_storeselect;
}

sub getimbox_b {
    my ( $act, $get_ims, $other_storeselect ) = @_;
    my %get_ims = %{$get_ims};
    my ( $status, $boxtxt ) = ( q{}, q{} );
    if ( $act eq 'imshow' && $INFO{'caller'} =~ /[256]/xsm ) {
        $act = $act . "_$INFO{'caller'}";
    }
    if ( $get_ims{$act} ) {
        (
            undef,       $mctitle,  $status,
            $senderinfo, $callerid, $boxtxt,
            $movebutton, $im_box,   $pmfile_toopen
        ) = @{ $get_ims{$act} };
    }
    if ( $act eq 'im'
        || ( $act eq 'imshow' && $INFO{'caller'} == 1 ) )
    {
        ( $status, $boxtxt ) = act_im($other_storeselect);
    }
    elsif ( $act eq 'imstorage'
        || ( $act eq 'imshow' && $INFO{'caller'} == 3 ) )
    {
        ( $status, $boxtxt ) = act_store($other_storeselect);
    }
...
sub view_folders {
    ...
    if ( ${$username}{'PMfolders'} ) {
        ...
        foreach my $storefolder ( split /[|]/xsm, ${$username}{'PMfolders'} ) {
            if ( $storefolder ne 'in' && $storefolder ne 'out' ) {
                ...
                $mc_pmmenu_temp =~ s/\Q{yabb storefolder}\E/$storefolder/gxsm;
                $mc_pmmenu_temp =~ s/\Q{yabb storefolderl}\E/$storefolderl/gxsm;
                $mc_pmmenu_temp =~
                  s/\Q{yabb MCPmMenuTemp_chk}\E/$mc_pmmenu_temp_chk/gxsm;
                $mc_pmmenu_temp =~ s/\Q{yabb foldercount}\E/$foldercount/gxsm;
                $x++;
            }
        }
    ...
    if ($storefolders_total) {
        ...
        $mc_pmmenu_strtot =~ s/\Q{yabb MCPmMenuTemp}\E/$mc_pmmenu_temp/xsm;
    }
    ...
    return ( $mc_pmmenu_strtot, $mc_pmmenu_newfolder );
}

sub view_bgmess {
    my ( $mc_pmmenu_bmbox, $mc_pmmenu_gmbox ) = ( q{}, q{} );
    ...
    return ( $mc_pmmenu_bmbox, $mc_pmmenu_gmbox );
}

sub act_im {
    my ($other_storeselect) = @_;
    $mctitle = $inmes_txt{'inbox'};
    my $status = $inmes_imtxt{'status'};
    $senderinfo = $inmes_txt{'318'};
    $callerid   = 1;
    my $boxtxt = $inmes_txt{'316'};
    $movebutton =
qq~<input type="submit" name="imaction" value="$inmes_imtxt{'store'}" class="button" />$other_storeselect $inmes_txt{'storeor'}~;
...
sub act_store {
    ...
    $callerid = 3;
    my $boxtxt = $inmes_txt{'storage'};
    $movebutton =
qq~<input type="submit" name="imaction" value="$inmes_imtxt{'store'}" class="button" />$other_storeselect $inmes_txt{'storeor'}~;
    $im_box = $inmes_txt{'storage'};

    my $folder = getfolder($username);
...
sub multi_1 {
    ...
    if ( $messlst{'mstatus'} !~ /b/xsm ) {
        ...
        foreach my $muser ( split /,/xsm, $messlst{'mtousers'} ) {
            $usertomess_read =
              check_ims( $muser, $messlst{'messageid'}, 'messageopened' );
...
qq~<a href="$scripturl?action=viewprofile;username=$useraccount{$muser}" rel="nofollow">$format_unbold{$muser}</a>~;
        ...
        if ( $messlst{'mccusers'} ) {
            ## check each to see if they read the message
            foreach my $muser ( split /,/xsm, $messlst{'mccusers'} ) {
                $usertomess_read =
                  check_ims( $muser, $messlst{'messageid'}, 'messageopened' );
                if ( !$yy_udloaded{$muser} ) {
                    load_user($muser);
        ...
        if ( $messlst{'mbccusers'} ) {
            ## check each to see if they read the message
            foreach my $muser ( split /,/xsm, $messlst{'mbccusers'} ) {
                $usertomess_read =
                  check_ims( $muser, $messlst{'messageid'}, 'messageopened' );
                if ( !$yy_udloaded{$muser} ) {
                    load_user($muser);
    ...
    return ( $usernameto, $usertomess_read );
}
...
sub multi_2 {
    ...
    if ( $messlst{'mstatus'} !~ /b/xsm ) {
        $usertomess_read =
          check_ims( $messlst{'mtousers'}, $messlst{'messageid'},
            'messageopened' );
...
sub get_im_icon {
    my ( $has_multirecs, $im_opened, $im_repliedto ) = @_;
    my $message_icon = q{};
    if ( !$has_multirecs ) {
        if ( !$im_opened ) {
            $message_icon =
...
sub get_out_icon {
    my ( $has_multirecs, $im_opened, $im_repliedto, $messlst ) = @_;
    my %messlst      = %{$messlst};
    my $message_icon = q{};
    my $call_back    = q{};
...
qq~<img src="$imagesdir/$newload{'imclose'}" alt="$inmes_imtxt{'outnotread'}" title="$inmes_imtxt{'outnotread'}" />~;
    ...
    if ($has_multirecs) {
        ...
        foreach my $recname ( split /,/xsm, $tousers ) {
            ...
            load_user($recname);
            if (
                check_ims( $recname, $messlst{'messageid'}, 'messageopened' )
...
qq~<a href="$scripturl?action=imshow;id=$messlst{'messageid'};caller=2">$inmes_imtxt{'multicallback'}</a> | ~;
    ...
    return ( $message_icon, $call_back );
}

sub get_user_to {
    my ($messlst) = @_;
    my %messlst = %{$messlst};
    $uname = $messlst{'mtousers'};
    my @usernameto;    # is to user
    if ( $messlst{'mccusers'} ) {
    ...
    foreach my $unam ( split /,/xsm, $uname ) {
        load_user($unam);
        push
          @usernameto,
          (
            ${ $uid . $unam }{'realname'}
            ? qq~<a href="$scripturl?action=viewprofile;username=$useraccount{$unam}" rel="nofollow">$format_unbold{$unam}</a>~
            : (
                  $unam ? qq~$unam ($maintxt{'470a'})~
                : $maintxt{'470a'}
            )
          );    # 470a == Ex-Member
    }
...
sub get_guest_to {
    ...
    my %messlst    = %{$messlst};
    my @usernameto = ();
    my ( $guest_name, $guest_email ) = split /[ ]/xsm, $messlst{'musername'};
    $guest_name =~ s/%20/ /gxsm;
    my $usernamefrom =
      qq~$guest_name<br />(<a href="mailto:$guest_email">$guest_email</a>)~;
    ...
    return ( @usernameto, $usernamefrom );
}
...
sub bm_action {
    ...
    my $actions_menu =
qq~<a href="$scripturl?action=imsend;caller=$callerid;quote=$messlst{'mreplyno'};to=$useraccount{$messlst{'musername'}};id=$messlst{'messageid'}$mymid">$inmes_txt{'145'}</a>$sepa<a href="$scripturl?action=imsend;caller=$callerid;reply=$messlst{'mreplyno'};to=$useraccount{$messlst{'musername'}};id=$messlst{'messageid'}$mymid">$inmes_txt{'146'}</a>$sepa<a href="$scripturl?action=imsend;caller=$callerid;forward=1;quote=$messlst{'mreplyno'};id=$messlst{'messageid'}">$inmes_txt{'147'}</a>$sepa<a href="$scripturl?action=deletemultimessages;caller=$callerid;deleteid=$messlst{'messageid'}" onclick="return confirm('$inmes_txt{'770'}')">$inmes_txt{'remove'}</a>~;
    return $actions_menu;
}

sub get_mycenter_mess {
    my ( $dmmessages, $toopen, $usr ) = @_;
    my @dmmessages = @{$dmmessages};
    my ( @bmessages, @gmessages );
    if ( !$INFO{'focus'} ) {
        ( @dmmessages, @gmessages, @bmessages ) =
          getdrawpmbox_1( $usr, $toopen );
    }
    ...
    push @dmmessages, @stkbmessages;
    push @dmmessages, @tmpbmessages;
    undef @stkbmessages;
    undef @tmpbmessages;
    undef @chbm;
    return @dmmessages;
...
sub get_legend {
    ...
    elsif ($three_weeksset
        && $gpdate > 21
        && $counter > $counter_check )
    {
        $three_weeksset = 0;
        if ( $monthset && $counter == 1 ) {
            if ( $gpdate <= 31 ) {
                $counter_check = $counter;
            }
            $uselegend = 'fourweeks';
    ...
    {
        $monthset = 0;
        if ( $gt_monthset && $counter == 1 ) { $uselegend = 'older'; }
    }
    return $uselegend;
}
