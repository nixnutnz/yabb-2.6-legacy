...
qq~					<option value="$bodystyle" selected="selected">$templ_txt{'25'}</option>\n~;
    ...
    if ( $stylestr =~ /\#container/sm ) {
        ...
        $containerstyle =~ s/.*?(\#container\s*?\{.+?\}).*/$1/igsm;
        $selstyl .=
    ...
    if ( $stylestr =~ /\.tabtitle/sm && $istabbed ) {
        ...
        if ( $stylestr =~ /\.tabtitle a/sm ) {
            $tabtitlestyle_a = $stylestr;
            $tabtitlestyle_a =~ s/.*?(\.tabtitle a\s*?\{.+?\}).*/$1/igsm;
            $selstyl .=
qq~					<option value='$tabtitlestyle_a'>$templ_txt{'tabtitlea'}</option>\n~;
    ...
    if ( $stylestr =~ /\.windowbg2/sm ) {
        ...
        $window2style =~ s/.*?(\.windowbg2\s*?\{.+?\}).*/$1/igsm;
...
qq~<img src="$imagesdir/tabfill.gif" alt="" style="vertical-align: middle;" />~;
	...
	<table>
    	<tr>
        	<td class="menutop" style="height:22px">&nbsp;</td>
		</tr>
	</table>
	<table>
		<tr>
...
<table class="bordercolor cs_1px pad_4px">
	<tr>
		...
		<td id="window1" class="windowbg vtop">&nbsp;</td>
		<td id="window2" class="windowbg2 vtop">
			$aquote
			$acode
			$aedit<br />
...
qq~<img src="$imagesdir/search.gif" alt="" style="border: 0; background-color: transparent; margin-right: 5px; vertical-align: middle;" />~;
    ...
    $fulltemplate =~ s/({|<)yabb charset(}|>)/$yycharset/gsm;
    ...
    $fulltemplate =~ s/({|<)yabb news(}|>)/$tempnews/gsm;
...
sub DisplayTempl {
    ...
    if ( $UseMenuType == 0 ) {
        ...
        $msnimg = qq~$menusep<img src="$x[1]/msn.gif" alt="" />~;
    }
    elsif ( $UseMenuType == 1 ) {
        ...
        $yimimg =
qq~$menusep<img src="$yyhtml_root/Buttons/$language/yim.png" alt="" />~;
        $aimimg =
qq~$menusep<img src="$yyhtml_root/Buttons/$language/aim.png" alt="" />~;
        $msnimg =
qq~$menusep<img src="$yyhtml_root/Buttons/$language/msn.png" alt="" />~;
    }
