...
sub ImgLoc {
    ...
    elsif ( -e "$forumstylesdir/$useimages/$img[0]" ) {
        ...
        if ( $maxsearchdisplay > -1 ) {
            $yysearchbox = qq~
                    ...
                    <form action="$scripturl?action=search2" method="post" accept-charset="$yycharset">
                        ...
                        <input type="image" src="$imagesdir/search.gif" title="$maintxt{'searchimg'} $showsearchboxnum $maintxt{'searchimg2'}" style="border: 0; background-color: transparent; margin-right: 5px; vertical-align: middle;" />
                    </form>
~;
        }
    }
...
qq~[$debug_txt{'name'}-&gt;]$name=@value\[&lt;-$debug_txt{'value'}]<br />~;
    ...
    if (   $INFO{'username'}
        ...
        && $action ne 'activate'
        && $action ne 'rej_regentry'
        && $action ne 'apr_regentry' )
    {
...
sub enc_eMail {
    ...
    if ( $yycharset eq 'windows-1251' ) {
        $charset_value = 848;
    }    # Cyrillic decoding (We are now using UTF-8 only)
...
sub FromChars {
    my ($f_chars) = @_;
    $f_chars =~ s/&#(\d{3,});/ $1>127 ? "[ch$1]" : $& /egism;
...
sub ToChars {
    my ($t_chars) = @_;
    $t_chars =~ s/\[ch(\d{3,})\]/ $1>127 ? "\&#$1;" : q{} /egism;
    return $t_chars;
}
...
sub guestLangSel {
    ...
    foreach my $filesanddirs ( sort { lc($a) cmp lc $b } @langDir ) {
            ...
            && ( -e "$langdir/$filesanddirs/Register.lng" ) )
        {
            ...
            $langopt .=
              qq~<option value="$filesanddirs"$lngsel>$displang</option>~;
            $morelang++;
        }
    }
    return $langopt;
