...
$yabbcpmver = 'YaBB 2.5.4 $Revision$';
...
sub MakeSmileys {
    ...
    $message =~
s/(\W|^)\[smil(ie|ey)=(\S+?\.(gif|jpg|png|bmp))\]/$1<img src="$yyhtml_root\/Smilies\/$3" alt="$post_txt{'287'}" title="$post_txt{'287'}" \/>/igsm;
    $message =~
s/(\W|^);-?\)/$1<img src="$imagesdir\/wink.gif" alt="$post_txt{'292'}" title="$post_txt{'292'}" \/>/gsm;
    $message =~
s/(\W|^);D/$1<img src="$imagesdir\/grin.gif" alt="$post_txt{'293'}" title="$post_txt{'293'}" \/>/gsm;
    $message =~
s/(\W|^):'\(/$1<img src="$imagesdir\/cry.gif" alt="$post_txt{'530'}" title="$post_txt{'530'}" \/>/gsm;

    #' make my text editor happy;
    $message =~
s/(\W|^):-\//$1<img src="$imagesdir\/undecided.gif" alt="$post_txt{'528'}" title="$post_txt{'528'}" \/>/gsm;
    $message =~
s/(\W|^):-X/$1<img src="$imagesdir\/lipsrsealed.gif" alt="$post_txt{'527'}" title="$post_txt{'527'}" \/>/gsm;
    $message =~
s/(\W|^):-\[/$1<img src="$imagesdir\/embarassed.gif" alt="$post_txt{'526'}" title="$post_txt{'526'}" \/>/gsm;
    $message =~
s/(\W|^):-\*/$1<img src="$imagesdir\/kiss.gif" alt="$post_txt{'529'}" title="$post_txt{'529'}" \/>/gsm;
    $message =~
s/(\W|^)&gt;:\(/$1<img src="$imagesdir\/angry.gif" alt="$post_txt{'288'}" title="$post_txt{'288'}" \/>/gsm;
    $message =~
s/(\W|^)::\)/$1<img src="$imagesdir\/rolleyes\.gif" alt="$post_txt{'450'}" title="$post_txt{'450'}" \/>/gsm;
    $message =~
s/(\W|^):P/$1<img src="$imagesdir\/tongue\.gif" alt="$post_txt{'451'}" title="$post_txt{'451'}" \/>/gsm;
    $message =~
s/(\W|^):-?\)/$1<img src="$imagesdir\/smiley\.gif" alt="$post_txt{'287'}" title="$post_txt{'287'}" \/>/gsm;
    $message =~
s/(\W|^):D/$1<img src="$imagesdir\/cheesy.gif" alt="$post_txt{'289'}" title="$post_txt{'289'}" \/>/gsm;
    $message =~
s/(\W|^):-?\(/$1<img src="$imagesdir\/sad.gif" alt="$post_txt{'291'}" title="$post_txt{'291'}" \/>/gsm;
    $message =~
s/(\W|^):o/$1<img src="$imagesdir\/shocked.gif" alt="$post_txt{'294'}" title="$post_txt{'294'}" \/>/gism;
    $message =~
s/(\W|^)8-\)/$1<img src="$imagesdir\/cool.gif" alt="$post_txt{'295'}" title="$post_txt{'295'}" \/>/gsm;
    $message =~
s/(\W|^):-\?/$1<img src="$imagesdir\/huh.gif" alt="$post_txt{'296'}" title="$post_txt{'296'}" \/>/gsm;
    $message =~
s/(\W|^)\^_\^/$1<img src="$imagesdir\/happy.gif" alt="$post_txt{'801'}" title="$post_txt{'801'}" \/>/gsm;
    $message =~
s/(\W|^):thumb:/$1<img src="$imagesdir\/thumbsup.gif" alt="$post_txt{'282'}" title="$post_txt{'282'}" \/>/gsm;
    $message =~
s/(\W|^)&gt;:-D/$1<img src="$imagesdir\/evil.gif" alt="$post_txt{'802'}" title="$post_txt{'802'}" \/>/gsm;
	...
	while ($SmilieURL[$count]) {
        ...
        $tmpcode =~ s/&#64;/\@/gxsm;
        $message =~
s/\Q$tmpcode\E/<img src="$tmpurl" alt="$SmilieDescription[$count]" title="$SmilieDescription[$count]" \/>/gxsm;
		$count++;
	}
...
s/(\[url[^\[]*\]\s*)?\[img(.*?)\](.*?)\[\/img\]/ imagemsg($1,$2,$3,$image_type) /eisgm
    ...
    $message =~
s/\[color=([A-Za-z0-9# ]+)\](.+?)\[\/color\]/<span style="display:inline; color: $1;">$2<\/span>/isgm;
    $message =~
s/\[black\](.*?)\[\/black\]/<span style="display:inline; color:#000000;">$1<\/span>/isgm;
    $message =~
s/\[white\](.*?)\[\/white\]/<span style="display:inline; color:#FFFFFF;">$1<\/span>/isgm;
    $message =~
s/\[red\](.*?)\[\/red\]/<span style="display:inline; color:#FF0000;">$1<\/span>/isgm;
    $message =~
s/\[green\](.*?)\[\/green\]/<span style="display:inline; color:#00FF00;">$1<\/span>/isgm;
    $message =~
s/\[blue\](.*?)\[\/blue\]/<span style="display:inline; color:#0000FF;">$1<\/span>/isgm;
    $message =~ s/\[timestamp\=([\d]{9,10})\]/timeformat($1)/eisgm;
    $message =~
s/\[font=([A-Za-z0-9# -]+)\](.+?)\[\/font\]/<span style="display:inline; font-family: $1;">$2<\/span>/isgm;
...
s/\[justify\](.*?)\[\/justify\]/<div style="text-align: justify">$1<\/div>/isgm;
    ...
    $message =~
s/\[fixed\](.*?)\[\/fixed\]/<div style="display:inline; font-family: Courier New;">$1<\/div>/isgm;
...
s/\[ftp\]\s*(ftp:\/\/)?(.+?)\s*\[\/ftp\]/<a href="ftp:\/\/$2">$1$2<\/a>/isgm;
    ...
    $message =~
      s/\[email\]\s*(\S+?\@\S+?)\s*\[\/email\]/<a href="mailto:$1">$1<\/a>/isgm;
    $message =~
s/\[email=\s*(\S+?\@\S+?)\](.*?)\[\/email\]/<a href="mailto:$1">$2<\/a>/isgm;

    $message =~
s~\[edit\](.*?)\[/edit\]~<b>$post_txt{'603'}: </b><br /><div class="editbg" style="overflow: auto;">$1</div>~isg;

    $message =~
s/\/me\s+(.*)/<i><span style="color: #FF0000;"><b>$displayname<\/b><\/span> $1<\/i>/igxsm;
