...
qq~<div style="float: left; font-size: 10px; font-family: verdana, sans-serif; overflow: auto; max-height: 200px; width: 99%;">${$uid.$user}{'signature'}</div>~;
    ...
    if ( $UseMenuType == 0 ) {
        $yimimg =
qq~<img src="$imagesdir/yim.gif" alt="${$uid.$user}{'yim'}" title="${$uid.$user}{'yim'}" />~;
        $aimimg =
qq~<img src="$imagesdir/aim.gif" alt="${$uid.$user}{'aim'}" title="${$uid.$user}{'aim'}" />~;
        $skypeimg =
qq~<img src="$imagesdir/skype.gif" alt="${$uid.$user}{'skype'}" title="${$uid.$user}{'skype'}" />~;
        $myspaceimg =
...
qq~<img src="$imagesdir/gtalk2.gif" style="cursor: pointer" onclick="window.open('$scripturl?action=setgtalk;gtalkname=$thegtalkuser','','height=80,width=340,menubar=no,toolbar=no,scrollbars=no'); return false" alt="$thegtalkname" title="$thegtalkname" />~;
        $icqimg =
qq~<img src="http://web.icq.com/whitepages/online?icq=${$uid.$user}{'icq'}&#38;img=5" alt="${$uid.$user}{'icq'}" title="${$uid.$user}{'icq'}" />~;
    }
    elsif ( $UseMenuType == 1 ) {
        $yimimg      = q~<span class="imgwindowbg">YIM</span>~;
        $aimimg      = q~<span class="imgwindowbg">AIM</span>~;
...
qq~<span class="imgwindowbg" style="cursor: pointer" onclick="window.open('$scripturl?action=setgtalk;gtalkname=$thegtalkuser','','height=80,width=340,menubar=no,toolbar=no,scrollbars=no'); return false">GTalk</span>~;
        ...
        $yimimg =
qq~<img src="$yyhtml_root/Buttons/$language/yim.png" alt="${$uid.$user}{'yim'}" title="${$uid.$user}{'yim'}" />~;
        $aimimg =
qq~<img src="$yyhtml_root/Buttons/$language/aim.png" alt="${$uid.$user}{'aim'}" title="${$uid.$user}{'aim'}" />~;
        $skypeimg =
qq~<img src="$yyhtml_root/Buttons/$language/skype.png" alt="${$uid.$user}{'skype'}" title="${$uid.$user}{'skype'}" />~;
        $myspaceimg =
...
qq~<img src="$yyhtml_root/Buttons/$language/gtalk.png" style="cursor: pointer" onclick="window.open('$scripturl?action=setgtalk;gtalkname=$thegtalkuser','','height=80,width=340,menubar=no,toolbar=no,scrollbars=no'); return false" alt="$thegtalkname" title="$thegtalkname" />~;
        $icqimg =
qq~<img src="$yyhtml_root/Buttons/$language/icq.png" alt="${$uid.$user}{'icq'}" title="${$uid.$user}{'icq'}" />~;
    }
    ...
    if ( $showgenderimage && ${ $uid . $user }{'gender'} ) {
        ...
        ${ $uid . $user }{'gender'} =
          ${ $uid . $user }{'gender'}
          ? qq~$load_txt{'231'}: <img src="$imagesdir/${$uid.$user}{'gender'}.gif" alt="${$uid.$user}{'gender'}" title="${$uid.$user}{'gender'}" /><br />~
          : q{};
    }
    else {
        ${ $uid . $user }{'gender'} = q{};
...
sub LoadBroadcastMessages {    #check broadcast messages
    ...
    if ( -e "$memberdir/broadcast.messages" ) {
        my %PMbcRead;
        map { $PMbcRead{$_} = 0; } split /,/xsm, ${$builduser}{'PMbcRead'};
