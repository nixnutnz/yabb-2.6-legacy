...
sub RSS_board {
	...
	my $i = 0;
	foreach (@threadlist) {
		($mnum, $msub, $mname, $memail, $mdate, $mreplies, $musername, $micon, $mstate, $ns) = split /\|/xsm, $_;
		$curnum = $mnum;
		...
		if ($accept_permalink){
			...
			$yymain .= q~		<item>
				...
				<link>~ . RSSDescriptionTrim("http://$perm_domain/$symlink$permdate/$currentboard/$curnum") . q~</link>
				<category>~ . RSSDescriptionTrim($category) . q~</category>
				<guid isPermaLink="true">~ . RSSDescriptionTrim("http://$perm_domain/$symlink$permdate/$currentboard/$curnum") . q~</guid>
~;
		} else {
			$yymain .= q~		<item>
				<title>~ . RSSDescriptionTrim($msub) . q~</title>
				<link>~ . RSSDescriptionTrim("$scripturl?num=$curnum") . q~</link>
				<category>~ . RSSDescriptionTrim($category) . q~</category>
				<guid>~ . RSSDescriptionTrim("$scripturl?num=$curnum") . q~</guid>
~;
		}
		...
		fclose(TOPIC);
		if ($post ne q{}) {
			(undef, undef, undef, undef, $musername, undef, undef, undef, $message, $ns) = split /\|/xsm, $post;
		}
		if ($showauthor) {
			if (-e "$memberdir/$musername.vars") {
				LoadUser($musername);
				if (!${$uid.$musername}{'hidemail'}){
					$yymain .= q~<author>~ . RSSDescriptionTrim("${$uid.$musername}{'email'} (${$uid.$musername}{'realname'})") . q~</author>~;
				}
				else {
					$yymain .= q~			<author><name>~ . RSSDescriptionTrim("(${$uid.$musername}{'realname'})") . q~</name></author>\n~;
				}
			}
		}
		if ($showdate) {
				if ($rss_message == 2) { $mdate = $curnum ;} # Sort by topic creation if requested.
...
~;
		...
		if ($message ne q{}) {
			...
			$yymain .= q~		<description>~ . RSSDescriptionTrim($message) . q~</description>
~;
		}
		# Finish up the item
		$yymain .= q~		</item>
...
sub RSS_recent {
	...
	foreach $catid (@categoryorder) {
		...
		foreach my $curboard (@bdlist) {
			($boardname{$curboard}, $boardperms, $boardview) = split /\|/xsm, $board{$curboard};
			...
			for my $i ( 0 .. ( $topics - 1 ) ) {
				...
				($mnum, undef, undef, undef, $mdate, undef, undef, undef, $mstate) = split /\|/xsm, $buffer;
				$mdate = sprintf '%010d', $mdate;
				if ($rss_message == 2) { $mdate = $mnum ;} # Sort by topic creation if requested.
	...
	for my $i ( 0 .. ( @threadlist - 1 )) {
		# Opening item stuff
		($mdate, $board, $mnum, $msub, $mname, $memail, $modate, $mreplies, $musername, $micon, $mstate) = split /\|/xsm, $threadlist[$i];
		$curnum = $mnum;
		...
		if ($accept_permalink){
			...
			<link>~ . RSSDescriptionTrim("http://$perm_domain/$symlink$permdate/$board/$curnum") . q~</link>
			<category>~ . &RSSDescriptionTrim($category) . q~</category>
			<guid isPermaLink="true">~ . RSSDescriptionTrim("http://$perm_domain/$symlink$permdate/$board/$curnum") . qq~</guid>\n~;
		} else {
			$yymain .= q~		<item>
			<title>~ . RSSDescriptionTrim("$bn - $msub") . q~</title>
			<link>~ . RSSDescriptionTrim("$scripturl?num=$curnum/$postid") . q~</link>
			<category>~ . RSSDescriptionTrim($category) . q~</category>
		...
		if ($showauthor) {
			...
			if (-e "$memberdir/$musername.vars") {
				&LoadUser($musername);
				if (!${$uid.$musername}{'hidemail'}){
					$yymain .= q~			<author>~ . RSSDescriptionTrim("${$uid.$musername}{'email'} (${$uid.$musername}{'realname'})") . qq~</author>\n~;
				}
				else {
					$yymain .= q~			<author><name>~ . RSSDescriptionTrim("${$uid.$musername}{'realname'}") . qq~</name></author>\n~;
				}
			}
		}
		...
		if ($message ne q{}) {
			...
			ToChars($message);
			$message = &Censor($message);
			$yymain .= q~			<description>~ . RSSDescriptionTrim($message) . qq~</description>\n~;
		}
...
sub RSS_template { # print RSS output
	...
	if ($INFO{'catselect'}) {$mainlink .= "?catselect=$INFO{'catselect'}" ;}

	my $tit = "$yytitle - $mbname";
	FromHTML($tit);
	my $descr = ($boardname ? "$boardname - " : q{}) . $mbname;
	FromHTML($descr);
	my $mn = $mbname;
	FromHTML($mn);
...
<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
	<channel>
		<atom:link href="$scripturl?action=$INFO{'action'}~ . ($INFO{'board'} ? ";board=$INFO{'board'}" : q{}) . q~" rel="self" type="application/rss+xml" />
		...
		<language>~ . RSSDescriptionTrim("$maintxt{'w3c_lngcode'}") . q~</language>
...
To do this, simply run "$sourcedir/RSS.pl (ARGUMENTS)". The RSS feed will be sent to STDOUT. (as a .pm, this will not work.)
...
Action to run. This is the exact same as the actions found in SubList.pm that belong to this file.
