...
qq~           <tr id="dropsubrow_$curboard" style="display: none">
                ...
                $templateblock =~ s/({|<)yabb boardname(}|>)/$boardname/gsm;
...
qq~<span class="small">$boardindex_txt{'143'}: <b>$numbots</b></span>~;
        ...
        ( $title, undef, undef, $color, $noshow, undef ) =
          split /\|/xsm, $Group{'Administrator'}, 6;
        if ( $color && $noshow != 1 ) {
            $tmpcnt++;
            $tmpcolors{$tmpcnt} =
...
qq~<option value="$maxrecentdisplay">$maxrecentdisplay</option>~;
                ...
                $recentpostslink .=
qq~</select> <input type="submit" style="display:none" /></form> $boardindex_txt{'792'} $boardindex_txt{'793'}~;
            }
