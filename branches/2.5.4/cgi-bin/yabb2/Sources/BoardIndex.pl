...
$boardindexplver = 'YaBB 2.5.4 $Revision: 1.0 $';
        ...
        if ( -e "$datadir/showcase.poll" ) {
            ...
            if ( -e "$datadir/$scthreadnum.poll" ) {
                ...
                if ( $iamadmin || $iamgmod ) {
                    ...
                    $pollthread =
                      &AccessCheck( ${$scthreadnum}{'board'}, '', $boardperms )
...
# make a copy of new boards has to update the tree if a sub board has a new post, but keep original so we know which individual boards are new
    ...
    foreach $catid (@tmplist) {
        ...
        if ( !$subboard_sel ) {
            ...
            &ToChars($boardname);
            ( $catname, $catperms, $catallowcol, $catimage ) =
              ( qq~$boardindex_txt{'65'} '$boardname'~, 0, 0, '' );
        }
...
# as we fill the vars based on all boards we need to skip any cat already shown before
                ...
                if ($catallowcol) {
                    ...
                    if ( $catcol{$catid} ) {
                        $newrowstart{$catname} =
qq~<tr><td colspan="5" class="$new_msg_bg" height="18"><span class="$new_msg_class">~;
...
qq~<img src="$imagesdir/$catimage" alt="" border="0" style="vertical-align: middle;" />~;
            ...
            $templatecat =~
              s/({|<)yabb newmsg start(}|>)/$newrowstart{$catname}/g;
            $templatecat =~
              s/({|<)yabb newmsg icon(}|>)/$newrowicon{$catname}/g;
            ...
            $templatecat =~
              s/({|<)yabb colboardtable(}|>)/$template_colboardtable/g;
                ...
                if ( $subboard{$curboard} ) {
                    ...
                    foreach $childbd (@childboards) {
                        ...
                        my ( $chldboardname, $chldboardperms, $chldboardview ) =
                          split( /\|/, $board{$childbd} );
                        my $access =
                          &AccessCheck( $childbd, '', $chldboardperms );
                        if (  !$iamadmin
                            && $access ne "granted"
                            && $chldboardview != 1 )
                        {
                        ...
                        if ($iamguest) {
                            ...
                            $sub_new =
qq~<img src="$imagesdir/sub_on.png" alt="$boardindex_txt{'333'}" title="$boardindex_txt{'333'}" border="0" style="vertical-align: middle;" />~;
                        }
                        else {
                            $sub_new =
qq~<img src="$imagesdir/sub_off.png" alt="$boardindex_txt{'334'}" title="$boardindex_txt{'334'}" border="0" style="vertical-align: middle;" />~;
                        }
...
qq~ $childcnt{$childbd} $boardindex_txt{'72'}~;
                        ...
                        $tmp_sublinks =~
                          s/({|<)yabb boardname(}|>)/$chldboardname/g;
                        $tmp_sublinks =~
s/({|<)yabb boardurl(}|>)/$scripturl\?board\=$childbd/g;
                        $tmp_sublinks =~ s/({|<)yabb new(}|>)/$sub_new/g;
                        $tmp_sublinks =~
                          s/({|<)yabb boardinfo(}|>)/$boardinfotxt/g;
...
# don't make an ajax dropdown if we are calling from ajax. All those dropdowns would get confusing.
                        if ( $INFO{'a'} ) {
                            ...
                            $subdropdown =
qq~<a href="javascript://" id="subdropa_$curboard" style="font-weight:bold" onclick="SubBoardList('$scripturl?board=$curboard','$curboard','$catid',$sub_count,$alternateboardcolor)"><img id="subdropbutton_$curboard" style="position: relative; top: 2px;" src="$imagesdir/sub_arrow.png" style="cursor: pointer;" border="0" />&nbsp;$sub_txt</a>~;
                        }
                    }
                    $tmp_sublist =~
                      s/({|<)yabb subboardlinks(}|>)/$template_subboards/g;
...
# if it's a parent board that cannot be posted in, just show sub board list when clicked vs. message index
                ...
                {
                    $templateblock =~
s/({|<)yabb boardurl(}|>)/$scripturl\?boardselect\=$curboard/g;
                }
                else {
                    $templateblock =~
                ...
                $templateblock =~
                  s/({|<)yabb expandmessages(}|>)/$expandmessages/g;
#                $templateblock =~
#                  s/({|<)yabb messagedropdown(}|>)/$messagedropdown/g;
                $templateblock =~
                  s/({|<)yabb messagedropdown(}|>)//g;
...
#        $boardindex_template =~ s/({|<)yabb catsblock(}|>)/$tmptemplateblock/g;
        ...
        $boardindex_template =~
          s/({|<)yabb boardhandellist(}|>)/$boardhandellist/g;
...
qq~</select> </form> $boardindex_txt{'792'} $boardindex_txt{'793'}~;
            ...
            $boardindex_template =~
              s/({|<)yabb lastpostlink(}|>)/$lastpostlink/g;
            $boardindex_template =~
              s/({|<)yabb recentposts(}|>)/$recentpostslink/g;
            $boardindex_template =~
              s/({|<)yabb lastpostdate(}|>)/$tmlsdatetime/g;
        }
        else {
        ...
        $boardindex_template =~
          s/({|<)yabb mostmembersdate(}|>)/$themostmembdate/g;
        $boardindex_template =~
          s/({|<)yabb mostguestsdate(}|>)/$themostguestdate/g;
        $boardindex_template =~
          s/({|<)yabb mostbotsdate(}|>)/$themostbotsdate/g;
        $boardindex_template =~
          s/({|<)yabb mostusersdate(}|>)/$themostuserdate/g;
...
sub MarkAllRead {    # Mark all boards as read.
    ...
    sub recursive_mark {
        foreach $board (@_) {
            ...
            # make recursive call if this board has more children
            if ( $subboard{$board} ) {
                &recursive_mark( split( /\|/, $subboard{$board} ) );
            }
        }
    }
