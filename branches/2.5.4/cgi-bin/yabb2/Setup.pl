...
$setupplver = 'YaBB 2.5.4 $Revision$';
...
EOF
    ...
    if ( !-e "$varsdir/news.txt" ) {
        ...
        print {NEWSFILE} "We've upgraded to YaBB 2.5.4!\n"
          or croak 'cannot print news.txt';
        print {NEWSFILE}
...
~;
    ...
    $yyim    = 'You are running YaBB 2.5.4 Setup.';
    $yytitle = 'YaBB 2.5.4 Setup';
    SetupTemplate();
    return;
}
...
'We are currently upgrading our forum again. Please check back shortly!';
        ...
        $max_post_img_width     = 65;
        $max_post_img_height    = 65;
        $fix_post_img_size      = 0;
        $max_signat_img_width   = 65;
        $max_signat_img_height  = 65;
        $fix_signat_img_size    = 0;
        $max_attach_img_width   = 65;
        $max_attach_img_height  = 65;
...
\$fix_avatarml_img_size = $fix_avatarml_img_size;                       # Set to 1 disable the image resize feature and sets the image size to the max_... values. If one of
                                                    #  the max_... values is 0 the image is shown in his proportions to the other value. If both are 0 the image is shown at his original size.
...
sub FoundSetupLock {
    ...
    if ( -e "$vardir/Converter.lock" ) {
        $conv = q{};
        $conv2 =
qq~The 1x to 2.5.4 Converter has already been run.<br />To run the Converter again, remove the file "$vardir/Converter.lock," then re-visit this page.~;
...
<!--                  <input type="hidden" name="formsession" value="$formsession" />-->
    ...
    $yyim    = 'YaBB 2.5.4 Setup has already been run.';
    $yytitle = 'YaBB 2.5.4 Setup';
    template();
    return;
}
